...
bool isPalindromeR(string s) {
	...
	bool check = (s.front() == s.back());


	return isPalindromeR(s.substr(1,s.length()-2)) && check;
}
...
string reverse(string s) {
	...
	return reverse(s.substr(1, s.length() - 1)) += front;
}
...
int sumDigits(int n) {
    ...
    return sumDigits(n / 10) + rem; // fix this!
}
...
int countOccurrences(const int* arr, int n, int x) {
    ...
    return countOccurrences(arr, n-1, x) + appears; // fix this!
}
